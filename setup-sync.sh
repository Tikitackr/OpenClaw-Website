#!/bin/bash
# ============================================================
# OpenClaw Sync-Server – Setup & Update Script
...
echo ""
echo "======================================"
echo "  OpenClaw Sync-Server Setup/Update"
echo "======================================"
echo ""

# --- Erkennung: Erstinstallation oder Update? ---
IS_UPDATE=false
if [ -f /root/sync-server/server.js ] && [ -f /root/sync-data/SYNC_TOKEN.txt ]; then
  IS_UPDATE=true
...
app.delete('/:filename', requireAuth, (req, res) => {
  const { filename } = req.params;
  if (!isValidFilename(filename)) return res.status(400).json({ error: 'Ungueltiger Dateiname' });
  const fp = path.join(DATA_DIR, filename);
  if (!fs.existsSync(fp)) return res.json({ ok: true, file: filename, message: 'Datei existierte nicht' });
  try {
    fs.unlinkSync(fp);
    console.log('Geloescht: ' + filename);
    res.json({ ok: true, file: filename, timestamp: new Date().toISOString() });
  } catch (e) { res.status(500).json({ error: 'Loeschfehler' }); }
});
app.use((req, res) => { res.status(404).json({ error: 'Nicht gefunden' }); });
app.listen(PORT, '0.0.0.0', () => {
  console.log('Sync-Server v1.1.0 laeuft auf Port ' + PORT);
});
SERVERJS
...
# --- 5. Token + systemd-Service ---
if [ "$IS_UPDATE" = true ]; then
  echo "[5/6] Update: Token beibehalten, Service neu starten..."
  SYNC_TOKEN=$(cat /root/sync-data/SYNC_TOKEN.txt)
  systemctl restart openclaw-sync
  echo "  ✓ Service neu gestartet (bestehender Token bleibt)"
else
...
echo "======================================"
if [ "$IS_UPDATE" = true ]; then
  echo "  ✓ Sync-Server UPDATE erfolgreich!"
else
